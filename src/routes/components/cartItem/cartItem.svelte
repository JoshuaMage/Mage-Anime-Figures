<script>
  import { cartItems, isCartVisible } from '$lib/store';
...
<div>
  {#if $isCartVisible}
    <div class="fixed inset-0 z-50 flex justify-end bg-black bg-opacity-70 font-[Roboto] text-white">
      <div class="relative w-[25rem] bg-gray-950">
        {#if $cartItems.length > 0}
          <h2 class="mb-10 text-start text-xl font-bold">
            <strong class="mr-3 text-2xl font-bold">My Cart</strong> ({$cartItems.length}) items
          </h2>

          <ul class="flex h-full max-h-[calc(100vh-10rem)] w-full flex-col justify-between overflow-y-auto pb-24">
            {#each $cartItems as item (item.id)}
              <li class="mb-5 w-full rounded-md bg-gray-800 py-5">
                <section class="mb-2 grid grid-cols-2">
                  ...
                  <section class="flex flex-col">
                    <p class="mb-1 text-start text-xs">
                      <span class={`${item.availability === 'Available' ? 'text-green-400' : 'text-red-400'} sm:text-[15px] md:text-[40px]`}>
                        .
                      </span>{item.availability}
                    </p>
                    <p class="text-wrap text-start font-bold">{item.description}</p>
                  </section>
                </section>
                ...
                <section class="flex flex-col gap-1">
                  ...
                  <section class="grid grid-cols-4 text-center text-[17px] font-bold">
                    ...
                    {#if item.showDeleteConfirmation}
                      <section class="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 transform rounded-lg border border-white bg-black bg-transparent px-20 py-10 text-white shadow-lg">
        ...
        <div class="absolute bottom-0 h-40 w-full content-center bg-black px-5 py-5">
          ...
          <section class="flex justify-between px-2">
            <button class="w-40 rounded-md border border-white py-2 active:border-2">View Cart</button>
            <button class="w-40 rounded-md border border-black bg-blue-950 py-2 active:opacity-90">Checkout</button>
          </section>
        </div>
