<script>
	// @ts-nocheck
	import { newFiguresAnime } from '../../newProducts/newFigures.js';
	...
	newFiguresAnime.forEach((characterGroup) => {
		Object.keys(characterGroup).forEach((name) => {
...
<div class="container grid grid-cols-4 grid-rows-3 my-5">
	{#each newFiguresAnime as characterGroup}
		{#each Object.entries(characterGroup) as [name, item]}
			<div >
				<div class="relative">
					<img
						src={item.image[currentIndexes[name]]}
						alt={`${name} image ${currentIndexes[name] + 1}`}
						class="h-[400px] w-[100%] object-fill px-1 mt-5"
					/>
					<button
				...
				<div class="relative flex ">
					<div class="flex-1">
						...
						<p class="break-words text-start text-xl font-bold italic pr-14">{item.description}</p>
